...
function Selectblock (sSelector) {
	...
	// Data
	var CloseBtn = '<div class="select-close"></div>';

	s.elem.append(CloseBtn);
	...
	s.setSelected = function(index) {
		...
		var el = options[index + 1];

		selectBlockItem.attr("data-value", $(el).attr("value"));
		selectBlockItem.html( $(el).html() );
	}
	s.resetSelected = function() {
		s.elem.find('.select-block__list li:first').trigger('click');
	}
	...
	s.closebtn.bind('click', s.globalClick);
}
